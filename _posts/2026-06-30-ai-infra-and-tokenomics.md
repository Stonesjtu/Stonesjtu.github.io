...
C_{\text{total}}(N) &\approx N \cdot R_{\text{user}} \cdot O(1)
\end{aligned}
$$
</div>

Here $R_{\text{user}}$ is the request rate generated by one user. In the pre-LLM loop, the user usually stays in the loop: click, watch, scroll, type, play, wait. That bounds how many expensive requests one person can trigger.
...
There is a second multiplier: request generation is no longer strictly user-paced. A user can define a goal once, then an agent can run for hours, call tools, inspect files, run tests, monitor state, and spawn multiple concurrent subtasks. If $A$ is the number of concurrent agents per user and $R_{\text{agent}}$ is the request rate inside each agent loop:

<div class="math-block">
$$
R_{\text{eff}} \approx R_{\text{user}} + A \cdot R_{\text{agent}}
$$
</div>

So post-LLM demand has two weak-scaling axes: the work per request grows, and the number of model requests triggered by each user goal can also grow.
...
$$
\begin{aligned}
\text{cost}_{\text{base}} &= N \cdot R_{\text{eff}} \cdot W_{\text{req}} \cdot c_{\text{compute}} \\
\text{cost}_{\text{opt}} &= N \cdot R_{\text{eff}} \cdot W_{\text{req}} \cdot c_{\text{compute}} \cdot r \\
\text{savings} &= N \cdot R_{\text{eff}} \cdot W_{\text{req}} \cdot c_{\text{compute}} \cdot (1-r)
\end{aligned}
$$
</div>

Under strong scaling, $W_{\text{req}} \approx O(1)$ and $R_{\text{eff}}$ is mostly user-paced, so savings mostly track traffic volume. Under weak scaling, $W_{\text{req}}$ grows with model size, context length, generated tokens, and agent steps, while $R_{\text{eff}}$ can grow with background agents and concurrent subtasks. The same 20% kernel, compiler, cache, or batching win is therefore magnified twice: by more work per request and by more requests per user goal. This is the "the more you buy, the more you save" property of AI infra: as token demand scales up, every percentage point of efficiency converts into a larger absolute dollar saving.
